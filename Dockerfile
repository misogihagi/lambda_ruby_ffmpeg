...
RUN echo proxy=http://192.168.1.2:3128 >> /etc/yum.conf
RUN echo proxy=http://192.168.1.2:3128 >> "$HOME/.curlrc"
RUN echo export http_proxy=http://192.168.1.2:3128 >> "$HOME/.bashrc"
RUN echo export HTTP_PROXY=http://192.168.1.2:3128 >> "$HOME/.bashrc"
RUN echo export https_proxy=http://192.168.1.2:3128 >> "$HOME/.bashrc"
RUN echo export HTTPS_PROXY=http://192.168.1.2:3128 >> "$HOME/.bashrc"
RUN echo [http_proxy] >> "$HOME/.hgrc"
RUN echo host = 192.168.1.2:3128/ >> "$HOME/.hgrc"
RUN echo always = true >> "$HOME/.hgrc"
RUN export HTTP_PROXY=http://192.168.1.2:3128 
RUN export http_proxy=http://192.168.1.2:3128 
RUN echo HTTP_PROXY: http://192.168.1.2:3128 >> "$HOME/.gemrc"
RUN echo http_proxy: http://192.168.1.2:3128 >> "$HOME/.gemrc"




RUN git config --global http.proxy http://192.168.1.2:3128

RUN gem install bundler -p http://192.168.1.2:3128
...
WORKDIR /var/task/ffmpeg_sources
RUN curl -O -L https://ffmpeg.org/releases/ffmpeg-snapshot.tar.bz2 --proxy http://192.168.1.2:3128
